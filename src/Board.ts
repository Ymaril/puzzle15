...
export class Board {
  ...
  constructor(private svg: SVGGraphicsElement, private size: number, state: number[][]) {
    state.forEach((row, y) => row.forEach((value, x) => value && this.createTile(value, { x, y })));
  ...
  private createTile(id: number, coordinates: Coordinates): SVGRectElement {
    ...
    Object
      .entries(this.getDefaultTileAttributes())
  ...
  private moveTile(tile: SVGRectElement, coordinates: Coordinates) {
    ...
    Object.entries({
      ...
      tile.setAttribute(axis, value.toString())
    );
  }
